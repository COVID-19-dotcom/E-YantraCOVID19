...
class UserDashboard : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val perlis = object : PermissionListener {
            override fun onPermissionGranted() {
                val contextView = findViewById<View>(android.R.id.content)
                val snack=Snackbar.make(contextView,"Permission granted to send SMS",Snackbar.LENGTH_LONG)
                snack.show()

            }
            override fun onPermissionDenied(deniedPermissions: MutableList<String>?) {
                val contextView = findViewById<View>(android.R.id.content)
                val snack=Snackbar.make(contextView,"Please Allow permission to send SMS",Snackbar.LENGTH_LONG)
                snack.show()
            }
        }
        TedPermission.with(this)
            .setPermissionListener(perlis)
            .setPermissions(android.Manifest.permission.SEND_SMS,android.Manifest.permission.RECEIVE_SMS)
            .check()
    ...
    private fun CoronaActive()
    {
            ...
            rdCoughDry.isChecked && (edtCoughDays.text.toString().toInt()>=3) && chkButtonDifficultyInBreathing.isChecked)
        {
            ...
            intent.putExtra("drname",edtDrNamename.text.toString())
            startActivity(intent)
        ...
        else
        {
            var string="0"
            var m=edtDrMobile.text.toString()
            var n=edtDrNamename.text.toString()
            var intent=Intent(this,UserResult::class.java)
            intent.putExtra("key1",string)
            intent.putExtra("key2",m)
            intent.putExtra("drname",n)
           startActivity(intent)

...
class CoronaInfo : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Handler().postDelayed({
            progressData.visibility = View.GONE
            coronaData.visibility = View.VISIBLE
        },3000)
    ...
    fun refresh(){
        Handler().postDelayed(object : Runnable {
            override fun run() {
                ...
                jsonParse()
                counttotal?.setInterpolator(AccelerateInterpolator())?.countAnimation(0, confrim)
                countconfirm?.setInterpolator(AccelerateInterpolator())?.countAnimation(0, total)
